...
where (est.titulo_proyecto_ley = pro.titulo_proyecto_ley or est.titulo_proyecto_ley = l.titulo_proyecto_ley) and est.nombre_comision = part.nombre_comision
...
from 
	...
	from bien_economico bien, bienes_del_legislador bLeg, legislador leg
	where bien.id_bien_economico = bLeg.id_bien_economico and bLeg.dni_legislador = leg.dni
		and bLeg.fecha_obtencion >= (select max(per.fecha_inicio) from periodos_del_legislador per where leg.dni = per.dni_legislador)
...
natural join
	...
	where bien.id_bien_economico = bLeg.id_bien_economico and bLeg.dni_legislador = leg.dni
		and bLeg.fecha_obtencion < (select max(per.fecha_inicio) from periodos_del_legislador per where leg.dni = per.dni_legislador)
		and (bLeg.fecha_sucesion is NULL or (bLeg.fecha_sucesion is not NULL and bLeg.fecha_sucesion > (select max(per.fecha_inicio) from periodos_del_legislador per where leg.dni = per.dni_legislador)))
	group by leg.dni) s2
group by s1.dni
